...
import { City, Country, Service } from 'types';
...
function PlanTreatment() {
  ...
  useEffect(() => {
    if (selectedCountry && selectedService) {
      ...
      const totalEstimatedPrice = {
        min:
          (treatments[country.code].meta.prices.treatment.min + transportPrice.min + accommodationPrice.min + insurancePrice.min) *
          duration *
          travelers,
        max:
          (treatments[country.code].meta.prices.treatment.max + transportPrice.max + accommodationPrice.max + insurancePrice.max) *
          duration *
          travelers,
      };
  ...
  const ButtonArea = () => {
    return activeStep === steps.length ? (
      ...
      <Flex justifyContent="end" alignItems="end" mt={4}>
        <Button
        ...
        <Button
          ...
          rounded="xl"
          color="white"
          boxShadow={'0 2px 10px 0px rgb(66 153 225 / 43%)'}
          pb="3px"
          //   h={40}
          // borderRadius="xl"
              ...
              <div className="row">
                  ...
                  <Box w={'full'} bg="rgba(255,255,255,1)" boxShadow={'2xl'} rounded={'md'} overflow={'hidden'} p="20">
                    <Steps size="lg" orientation="vertical" activeStep={activeStep}>
                      <Step width="100%" label={steps[0].label} key={0} description={steps[0].description}>
                        <Flex rounded="lg" width="100%" my={[2, 3, 6]}>
                          <TreatmentPlanningForm
                            countries={countries}
                            services={services}
                            surgeries={selectedService?.surgeryPackages || []}
                            durationSliderValue={durationSliderValue}
                            travelersSliderValue={travelersSliderValue}
                      ...
                      <Step width="100%" label={steps[1].label} key={1} description={steps[1].description}>
                        {selectedCountry && (
                          <TreatmentPricesComparison
                            cities={treatments[selectedCountry.code].cities}
                ...
                <div className="col-md-4">
                  <TreatmentPriceSummary
                    priceText={`${estimatedPrice.min}$ - ${estimatedPrice.max}$`}
                    features={summary}
                    nextButtonArea={ButtonArea}
                  />
                </div>
              </div>
            </div>
