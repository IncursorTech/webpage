...
export default function SignupCard() {
  ...
  const [dividerArray, setDividerArray] = useState(0);
  const [displayPage, setDisplayPage] = useState('none');
  const [displayAnalysisPage, setDisplayAnalysisPage] = useState('none');
  ...
  useEffect(() => {
    setQuestionArray(questions.slice(dividerArray, 5 + dividerArray));
    setDataPie({
      labels: [
        'ELEKTRİK ELEKTRONİK TEKNOLOJİLERİ',
        'BÜRO YÖNETİMİ',
  ...
  }, [dividerArray, dataSet]);

  const modalSignIn = () => {
    return (
      <Modal onClose={onClose} isOpen={isOpen} isCentered>
        <ModalOverlay bg="blackAlpha.300" backdropFilter="blur(10px) hue-rotate(90deg)" />
        <ModalContent maxW={{ base: '95vw', md: '40vw' }} mt={{ base: 150, md: 100 }} p={5}>
          <ModalHeader>Kayıt Oluştur</ModalHeader>
          <ModalCloseButton />
          <ModalBody pb={6}>
            ...
            <SimpleGrid columns={2} gap={5}>
              ...
              <Input h={'5rem'} size={'lg'} placeholder="Mail" onChange={(e) => setMail(e.target.value)} />
              <Select h={'5rem'} size={'lg'} placeholder="Sınıf" onChange={(event) => setClassNumber(+event.target.value)}>
                <option value="option1">8.Sınıf</option>
                <option value="option2">9.Sınıf</option>
                <option value="option3">10.Sınıf</option>
              </Select>
              ...
              <Input h={'5rem'} size={'lg'} placeholder="Okulu" />
            </SimpleGrid>
            <Divider orientation="horizontal" my={15} color={'gray.100'} />
            <Text py={5} fontWeight={'bold'}>
              Giriş Bilgiler
            </Text>
            <SimpleGrid columns={1} gap={5}>
              <Input h={'5rem'} size={'lg'} placeholder="Kullanıcı Adı" />
              <Stack direction={'row'}>
                <Input type={'password'} h={'5rem'} size={'lg'} placeholder="Şifre" />
                <Input type={'password'} h={'5rem'} size={'lg'} placeholder="Şifre Tekrar" />
              </Stack>
            </SimpleGrid>
            <Checkbox
              size={'lg'}
            ...
            >
              <Text pl={5} fontSize={'sm'}>
                Kişisel bilgilerimin SEDER tarafından Gizlilik Politikasında belirtilen hüküm ve koşullara uygun olarak saklanmasını ve
                işlenmesini kabul ediyorum <b> Gizlilik Politikası </b>
              </Text>
            </Checkbox>
          </ModalBody>
          <ModalFooter>
            <Button h={'5rem'} size={'lg'} colorScheme="blue" mr={3} onClick={() => handleSubmit()}>
              Kayıt
            </Button>
            <Button h={'5rem'} size={'lg'} onClick={onClose}>
              İptal
            </Button>
          </ModalFooter>
        </ModalContent>
        ...
        <ModalContent maxW={{ base: '95vw', md: '40vw' }} mt={{ base: 150, md: 100 }} p={5}>
          ...
          <ModalFooter>
            <Button
              ...
              colorScheme="blue"
              mr={3}
              onClick={() => (setDisplayLogin('none'), setDisplayPage('flex'), onClose())}
            >
              Giriş
            </Button>
          </ModalFooter>
  ...
  const analysisPage = () => {
    <Box>
      <Flex
        p={5}
        display={displayAnalysisPage}
        minH={'100vh'}
        align={'center'}
        justify={'center'}
        bgGradient="linear(to-l, #0077ffaa, #ff002baa)"
      >
        <Stack
          spacing={5}
          p={10}
  ...
  return (
    <Box>
      <Flex p={5} display={displayPage} minH={'100vh'} align={'center'} justify={'center'} bgGradient="linear(to-l, #0077ffaa, #ff002baa)">
        <Stack spacing={5} p={10} rounded={'3xl'} bg={'#f5f5f5'} shadow={'xl'} textAlign={'center'} minW={{ base: '100%', md: '60%' }}>
          <Stack minH={'2xl'} display={questions.length < dividerArray ? 'none' : 'grid'} minW={{ base: 'auto', md: '3xl' }}>
            {questionArray.map((item, index) => (
              ...
              <SimpleGrid
                ...
                fontWeight={'bold'}
              >
                <Text width={'100%'} display={{ base: 'grid', md: 'none' }} textAlign={'left'} pb={{ base: 15, md: 0 }}>
                  {item.question}
                </Text>
                <Stack display={'flex'} justifyContent={'flex-end'} spacing={5} direction="row" pb={{ base: 15, md: 0 }}>
                  <Text width={'100%'} display={{ base: 'none', md: 'grid' }} textAlign={'left'}>
                    {item.question}
                  </Text>
                  <Flex gap={5} align={'center'} justify={'center'}>
                    <Button
                      ...
                      onClick={() => {
                        const dt = dataSet;
                        item.code === 1 ? (dt[item.code] = dt[item.code] + 2) : (dt[item.code] = dt[item.code] + 1);
                        setDataSet(dt);
                      }}
                    >
                      Evet
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        item.code === 1
                          ? dt[item.code] === 0
                            ? (dt[item.code] = 0)
                            : (dt[item.code] = dt[item.code] - 2)
                          : dt[item.code] === 0
                          ? (dt[item.code] = 0)
                          : (dt[item.code] = dt[item.code] - 1);
                        setDataSet(dt);
                      }}
                    >
                      Hayır
          ...
          {questions.length < dividerArray ? (
            <>
              <Text width={'100%'} px={5} fontSize={'2xl'} color={'gray.600'} textTransform={'capitalize'} textAlign={'center'}>
                Teşekkürler Testi Tamamladınız <br></br> Kontrollerinizide Tamamladıysanız Testinizin Sonucunu Görmek İçin Aşağıdaki
                Bilgileri Doldurup <br></br> <b> Gönder </b> Butonuna Basınız.
              </Text>
              <Box px={{ base: '1rem' }}>
                <SimpleGrid columns={2} gap={5}>
                  <Input h={'5rem'} size={'lg'} placeholder="Adınız" onChange={(e) => setName(e.target.value)} />
            ...
            <Text px={5} fontSize={'sm'} color={'gray.600'} textTransform={'capitalize'} textAlign={'center'} w={'30rem'}></Text>
          )}
          <HStack justifyContent={'center'}>
            <Button
              ...
              isDisabled={dividerArray === 0}
              onClick={() => {
                setDividerArray(dividerArray - 5);
              }}
            ></Button>
            <Button
              h={'5rem'}
              ...
              isDisabled={questions.length < dividerArray}
              onClick={() => {
                setDividerArray(dividerArray + 5);
              }}
            ></Button>
        ...
        {!openState ? modalSignIn() : modalLogIn()}
      </Flex>
      <Flex display={displayLogin} minH={'100vh'} align={'center'} justify={'center'} bgGradient="linear(to-l, #0077ffaa, #ff002baa)">
        <Stack
          spacing={8}
          mx={'auto'}
          maxW={'4xl'}
          align={'center'}
          bg={{ base: '#f1f1f188', md: '#f1f1f1' }}
          shadow={'xl'}
          p={10}
          rounded={{ base: 'none', md: '3xl' }}
          m={{ base: 5, md: 0 }}
        >
          <Image src="/images/Picture2.png" alt="download_app" objectFit="contain" width={280} height={100} />
          ...
          <Stack spacing={25} p={10} rounded={'3xl'} bg={'#f5f5f5'} shadow={'xl'} textAlign={'center'}>
            <Box mb={55}>
              <Text fontSize={'3rem'}>
                <b> KAMEB</b>
              </Text>
          ...
          <NextLink href={'https://www.incursor.com'} passHref>
            <Stack direction={'row'} align={'center'} justify={'center'} cursor={'pointer'} py={15}>
              <Text px={5} fontSize={'sm'} color={'gray.600'} textTransform={'capitalize'} textAlign={'center'}>
                Created By
              </Text>
              <Image src="/images/incursor.png" alt="download_app" objectFit="contain" width={100} height={20} />
            </Stack>
          </NextLink>
        </Stack>
        {!openState ? modalSignIn() : modalLogIn()}
