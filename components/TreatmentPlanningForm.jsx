...
function TreatmentPlanningForm({ nextButtonArea, countries, services, surgeries, durationSliderValue, travelersSliderValue, onChangeCountry, onChangeService, onChangeSurgery, onChangeDurationSlider, onChangeTravelersSlider, onChangeTransportSelected, onChangeAccommodationSelected, onChangeInsuranceSelected, }) {
    return (<div className="container">
      ...
      <div className="row t">
        <div className="row my-2">
          ...
          <div className="col-md-8 py-2">
            <Select placeholder="Select" size="lg" onChange={(event) => onChangeCountry(+event.target.value)}>
              {countries.map((country, i) => (<option key={i} value={i}>
                  {country.name}
                </option>))}
            </Select>
          </div>
        ...
        <div className="row my-2">
          ...
          <div className="col-md-8 py-2">
            <Select placeholder="Select" size="lg" onChange={(event) => {
            onChangeService(+event.target.value);
        }}>
              {services.map((service, i) => (<option key={i} value={i}>
                  {service.name}
                </option>))}
            </Select>
          </div>
        ...
        <div className="row my-2">
          ...
          <div className="col-md-8 py-2">
            <Select placeholder="Select" size="lg" onChange={(event) => onChangeSurgery(+event.target.value)}>
              ...
              {surgeries.map((surgery, i) => (<option key={i} value={i}>
                  {surgery.name}
                </option>))}
            </Select>
          </div>
        ...
        <div className="row my-2">
          ...
          <div className="col-md-8 py-2 d-flex">
            <Switch colorScheme="blue" size="lg" onChange={(event) => {
