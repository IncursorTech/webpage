import { CheckIcon, ChevronDownIcon, ChevronRightIcon, CloseIcon, HamburgerIcon, SmallAddIcon } from '@chakra-ui/icons';
import {
  Box,
  Button,
  Collapse,
...
type NavbarContainerProps = { hidden: boolean; transparent: boolean; static: boolean };
...
export default function WithSubnavigation() {
  ...
  const navbarSelector = useSelector((storeState: RootState) => storeState.navbarState);
  ...
  const scrollPositionCallback = ({ currPos }: ScrollPositionEffectProps) => {
      ...
      <Box px={'3rem'}>
        ...
        >
          ...
          <Flex flex={{ base: 1, md: 'auto' }} ml={{ base: -2 }} display={{ base: 'flex', md: 'none' }}>
            <IconButton
              onClick={onToggle}
              icon={isOpen ? <CloseIcon w={3} h={3} /> : <HamburgerIcon color={'white'} w={18} h={18} />}
              variant={'ghost'}
