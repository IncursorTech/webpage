...
export interface SkillSectionProps {
  ...
  column: number;
  href: string;
  formItems: Array<MetaSkillFormItem>;
  type?: string;
}

export default function SkillSection({ type, column, href, formItems, buttonText }: PropsWithChildren<SkillSectionProps>) {
  return (
    <MainContainer>
          ...
          {formItems.map((item, index) => (
            <div key={index} className={`col-md-${12 / column}`}>
              <MetaSkillForm
                ...
                href={`${href}?${type ? `type=${type}` : ''}${index != null ? `&key=${index}` : ''}`}
                type={type}
                keyNumber={index}
              />
            </div>
