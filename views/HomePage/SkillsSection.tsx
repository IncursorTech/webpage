...
const MetaSkillFormItems: Array<MetaSkillFormItem> = [
  {
    title: 'Form Section',
    href: '#',
...
// `col-md-4`
export default function SkillSection({ column, subColumn }: PropsWithChildren<SkillSectionProps>) {
  return (
    <div>
      <div className="row">
        <div className={column}>
          <MetaSkillFormContainer>
            <div className="row">
              {MetaSkillFormItems.map((MetaItem, index) => (
                <div key={index} className={subColumn}>
                  <MetaSkillForm
                    title={MetaItem.title}
                    image={MetaItem.image}
                    description={MetaItem.description}
                    bgColor={MetaItem.background}
                  />
                </div>
              ))}
            </div>
